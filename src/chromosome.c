  ...
  while (improved) {
    ...
    for (int i = 1; i < chr->length - 1; i++) {
      for (int j = i + 1; j < chr->length; j++) {
        ...
        int left = i, right = j;
        while (left < right) {
          uint8_t tmp = chr->cities[left];
          chr->cities[left] = chr->cities[right];
          chr->cities[right] = tmp;
          left++; right--;
        }

        int new_fitness = compute_fitness(chr->cities, tsp);

        if (new_fitness < chr->fitness) {
          chr->fitness = new_fitness;
          improved = true;
        } else {
          left = i; right = j;
          while (left < right) {
